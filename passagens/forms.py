...
class PassagemForms(forms.ModelForm):
    data_pesquisa = forms.DateField(label='Data da pesquisa', disabled=True, initial=datetime.today) # consegue ver mas não alterar
    ...
    def clean(self):
        ...
        if lista_de_erros is not None:
            for erro in lista_de_erros: # for key in dict
                mensagem_erro = lista_de_erros[erro] # só cabe um erro por campo
...
class PessoaForms(forms.ModelForm):
    class Meta:
        model = Pessoa
        exclude = ['nome'] # inclui todos os campos, exceto o nome